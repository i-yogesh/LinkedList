...
public:
    Node *copyList(Node *head)
    {
        ...
        // Step2: Add CloneNodes between Original Nodes
        ...
        while (temp != NULL)
        {
            if (temp->next != NULL)
            {
                temp->next->arb = temp->arb ? temp->arb->next : temp->arb; // temp->arb here means NULL
            }
            temp = temp->next->next;
        }

        // Revert the process done in step 2
